describe('Stage admin feature', () => {
    it('Should be able to create repositories', () => {
      ...
      cy.visit("https://lca-stage.nal.usda.gov/lca-collaboration/");
       ...
       cy.url().should('contain', 'https://lca-stage.nal.usda.gov/lca-collaboration/administration/user/new');
       cy.get('#username').type('binquantestusername');
       cy.get('#name').type('binquantestname');
       cy.get('#email').type('binquan.wang@dsfederal.com');
       cy.get('#user-form > button').click();
       cy.url().should('contain', 'https://lca-stage.nal.usda.gov/lca-collaboration/administration/overview');
       cy.findAllByText("binquantestname").should("be.visible").click();
       cy.url().should('contain', 'https://lca-stage.nal.usda.gov/lca-collaboration/administration/user/profile/binquantestusername');
       cy.get('#main > div > div > div > div:nth-child(1) > button:nth-child(2)').click();
       cy.get('#confirmation-phrase').type('binquantestusername');
