...
@pytest.mark.parametrize("group,repo", [("Federal_LCA_Commons", "test_repo"), ("Federal_LCA_Commons", "another_repo")])
def test_get_repository_info(headers, group, repo):
...
@pytest.mark.parametrize("group", ["Federal_LCA_Commons", "test_group"])
def test_list_accessible_groups(headers, group):
    url = f"{BASE_URL}/ws/group/{group}"
    response = requests.get(url, headers=headers)
    assert response.status_code in [200, 404], f"Unexpected response code: {response.status_code}"

@pytest.mark.parametrize("group,repo,type,refId", [
    ("Federal_LCA_Commons", "test_repo", "FLOW", "flow_id"),
    ("Federal_LCA_Commons", "test_repo", "UNIT_GROUP", "unit_group_id")
])
def test_download_dataset(headers, group, repo, type, refId):
    url = f"{BASE_URL}/ws/public/browse/{group}/{repo}/{type}/{refId}"
    response = requests.get(url, headers=headers)
    assert response.status_code == 200, f"Failed to download dataset: {response.text}"
