...
public class Activities {
	...
	public static ObjectMap map(Commit commit, String repositoryPath) {
	...
	public static List<ObjectMap> map(Task task) {
		List<ObjectMap> activities = new ArrayList<>();
		activities.add(map(task, task.startDate, ActivityType.TASK_STARTED));
		...
		if (task.state == TaskState.COMPLETED || task.state == TaskState.CANCELED) {
			...
			activities.add(map(task, task.endDate, type));
		}
		for (TaskAssignment assignment : task.assignments) {
			activities.addAll(map(task, assignment));
		}
		return activities;
	}

	private static ObjectMap map(Task task, Date date, ActivityType type) {
		ObjectMap map = new ObjectMap();
		map.put("type", type);
	...
	private static List<ObjectMap> map(Task task, TaskAssignment assignment) {
		List<ObjectMap> activities = new ArrayList<>();
		activities.add(map(task, assignment, assignment.startDate, ActivityType.TASK_ASSIGNED));
		if (assignment.endDate != null) {
			ActivityType type = assignment.canceled ? ActivityType.TASK_ASSIGNMENT_CANCELED
					: ActivityType.TASK_ASSIGNMENT_COMPLETED;
			activities.add(map(task, assignment, assignment.endDate, type));
		}
		return activities;
	}

	private static ObjectMap map(Task task, TaskAssignment assignment, Date date, ActivityType type) {
		ObjectMap map = new ObjectMap();
		map.put("type", type);
