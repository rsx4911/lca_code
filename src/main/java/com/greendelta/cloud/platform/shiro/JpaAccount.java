...
public class JpaAccount implements Account, SaltedAuthenticationInfo {
	...
	public JpaAccount(User user, Realm realm) {
		Sha256Hash hash = Sha256Hash.fromHexString(user.getHash());
		ByteSource salt = new SimpleByteSource(Hex.decode(user.getSalt()));
		authenticationInfo = new SimpleAuthenticationInfo(user.getName(), hash, salt, realm.getName());
		roles = new ArrayList<>();
	}
